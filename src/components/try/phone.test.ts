...
const matches = (query: string, device: Device) =>
  query.split(',').some((part) => {
    const width = /max-width:\s*(\d+)px/.exec(part)
    if (width) return device.width <= Number(width[1])
    if (/pointer:\s*coarse/.test(part)) return device.coarse
    throw new Error(`the stub does not know ${part.trim()}`)
  })
...
describe('what counts as a phone', () => {
  ...
  it('is a phone at any width when the pointer is a finger', async () => {
    expect((await observe({ width: 1024, coarse: true })).at(-1)).toBe(true)
  })
