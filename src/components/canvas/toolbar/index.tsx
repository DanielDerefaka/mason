...
import type { useCanvasImage } from '@/hooks/use-canvas-image'
import { ToolBarShapes } from './shapes'
import { historyKeys, Tip, useModifierKey } from './tip'
 ...
 * it sits with the tools but opens a file picker instead of arming one. The
 * picker's hook belongs to the canvas, which owns the drop and paste paths
 * too: all three used to be separate instances of it, so what the canvas did
 * to a dropped image, the toolbar's picker did not do to a chosen one.
 */
export const ToolBar = ({
  ...
  zoom: ZoomControls
  images: ReturnType<typeof useCanvasImage>
}) => {
  const { tool, setTool, undo, redo, canUndo, canRedo } = useInfiniteCanvas()
  const { input, uploading, pick, place } = images
  const keys = historyKeys(useModifierKey())
    ...
    <TooltipProvider delayDuration={300}>
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-50 flex flex-wrap items-end justify-between gap-3 p-4 sm:flex-nowrap sm:p-5">
        <div className={`pointer-events-auto order-2 sm:order-1 ${track}`}>
          <Tip label="Undo" shortcut={keys.undo}>
            <Button
              variant="ghost"
              size="icon"
              aria-label="Undo"
          ...
          <Tip label="Redo" shortcut={keys.redo}>
            <Button
              variant="ghost"
              size="icon"
              aria-label="Redo"
